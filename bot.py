...
def updateCallback(client, callback_query,redis):
	...
	if date == "bkk":
		kup = InlineKeyboardMarkup([[InlineKeyboardButton("تعيين اميل",callback_data="setmail"),InlineKeyboardButton("تعيين عنوان",callback_data="setTit"),InlineKeyboardButton("تعيين كليشة",callback_data="settxt")],[InlineKeyboardButton("بدء spam",callback_data="spam")],[InlineKeyboardButton("قناة البوت",url="T.ME/TM_BOYKA")]])
		redis.delete("{}:{}:Type".format(BOT_ID,userID))
		app.answer_callback_query(callback_query.id, text="تم اللغاء الامر بنجاح")
		app.edit_message_text(chatID, message_id,"• اهلا بك في بوت الايميلات .\n• يمكنك ارسال رسائل بريدية عن طريق اضافه اميلات .\n[Khalid](t.me/E_M_K)", disable_web_page_preview=True, parse_mode=ParseMode.MARKDOWN,reply_markup=kup)
	if date == "spam":
...
def updateHandlers(client, message,redis):
	...
	if userID == SUDO:
		if text ==  '/start':
			kup = InlineKeyboardMarkup([[InlineKeyboardButton("تعيين اميل",callback_data="setmail"),InlineKeyboardButton("تعيين عنوان",callback_data="setTit"),InlineKeyboardButton("تعيين كليشة",callback_data="settxt")],[InlineKeyboardButton("بدء spam",callback_data="spam")],[InlineKeyboardButton("قناة البوت",url="T.ME/TM_BOYKA")]])
			message.reply_text("• اهلا بك في بوت الايميلات .\n• يمكنك ارسال رسائل بريدية عن طريق اضافه اميلات .\n[Khalid](t.me/E_M_K)", quote=True, disable_web_page_preview=True, parse_mode=ParseMode.MARKDOWN,reply_markup=kup)
